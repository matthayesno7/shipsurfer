...
/** Exchange a refresh token for a fresh access token (Supabase tokens expire). */
export async function refresh(refreshToken: string): Promise<{
  ...
  if (!data.access_token) {
    throw new Error(data.error_description || data.message || "Supabase token refresh failed");
  }
